...
사용법:
    ...
    python scripts/paper.py pdf  <arxiv_id> [--out papers/<category>]
...
def download_pdf(paper_id: str, outdir: str) -> str:
    ...
    if not bare:
        die("PDF 다운로드는 arXiv ID만 지원한다 (예: 1706.03762). "
            "그 외 논문은 출판사 페이지에서 직접 받아 해당 papers/<category>/ 에 두세요.")
    meta = get_meta(bare)
...
def main() -> None:
    ...
    p.add_argument("id")
    p.add_argument("--out", default="papers/other",
                   help="저장 폴더 (예: papers/wifi-csi; 기본값: papers/other)")

...
- PDF: [papers/category/file.pdf](../papers/category/file.pdf)
- 리뷰: [reviews/file.md](../reviews/file.md) 또는 `없음`
- 이유: {사용자가 말한 이유 또는 간단한 기록}
```

기존 항목은 보존하고, 제목·식별자·PDF 경로를 이용해 중복을 막는다. 메타데이터를 확인하지 못한 값은 추측하지 말고 `[확인 필요]`로 표시한다. PDF 경로는 현재 저장소의 카테고리 구조인 `papers/{wifi-csi|game-ai|agent-ai|computer-vision|other}/`를 따른다.
...
- **검증·동기화:** 모든 PDF·리뷰 링크의 존재를 확인하고, 이동된 카테고리별 PDF 경로를 현재 위치로 갱신한다. 논문 내용이나 평점을 임의로 갱신하지 않는다.

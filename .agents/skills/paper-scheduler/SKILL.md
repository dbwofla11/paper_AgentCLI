...
논문 주제는 `WiFi CSI`, `게임 AI (Game AI)`, `에이전트 AI (Agent AI)`, `컴퓨터 비전 (Computer Vision)` 중 하나 이상이어야 한다. 이 목록 밖의 주제(예: 일반 멀티모달)는 해당 허용 카테고리로 직접 구체화하도록 안내하거나 예약하지 않는다. 특정 논문 제목/arXiv ID를 예약하더라도 daily-digest-loop의 학회 발표·게재 확인을 통과해야 실제 회차에 포함된다.

...
4. `notes/trends/{날짜}.md`에 저장한다. **모든 일일 다이제스트 문서는 `notes/trends/2026-09-07.md`를 표준 양식으로 삼아 그 구조와 섹션 순서를 따른다.** 해당 기준 문서는 새 회차를 작성하기 전에 반드시 읽는다. 날짜·논문·이슈만 바꾸고, 요약형 bullet 나열이나 임의의 축약 양식으로 대체하지 않는다.

...
1. Asia/Seoul 기준 `YYYY-MM-DD`를 확정한다. `03-Trends/daily/{날짜}.md`가 이미 있으면 먼저 읽고, 기존 사실과 출처를 보존하면서 필요한 경우에만 갱신한다. 덮어쓸 때는 새 근거가 확인된 부분만 바꾼다. `03-Trends/daily/{날짜}-plan.md`가 있으면 해당 날짜에만 적용하는 사용자 지정 논문 선정 계획으로 읽고, 논문 슬롯 수와 주제 배분을 우선 준수한다. 계획 파일이 없는 날짜에는 일반적인 최신 논문 선정으로 진행한다.
...
4. `03-Trends/daily/{날짜}.md`에 저장한다. **모든 일일 다이제스트 문서는 `03-Trends/daily/2026-09-07.md`를 표준 양식으로 삼아 그 구조와 섹션 순서를 따른다.** 해당 기준 문서는 새 회차를 작성하기 전에 반드시 읽는다. 날짜·논문·이슈만 바꾸고, 요약형 bullet 나열이나 임의의 축약 양식으로 대체하지 않는다.
   - 문서 머리는 `# AI/ML/CS·시사 동향 다이제스트`, `기준일`, `구성`, `논문 선정 범주`, `원문 확인`을 포함한다.
   - `## 논문 3편` 아래에서 각 논문은 번호·정식 제목, 카테고리, 저자, 학회·연도·발표/게재 확인, 출처, 저장소 PDF 경로(있을 경우)를 먼저 적는다.
   - 각 논문에는 순서대로 `#### 핵심 요약`, `#### 중요한 이유`, `#### 주장–근거 대응`, `#### 한계`를 넣는다. 주장–근거 대응은 표로 작성하고, 수치·설정·사실에는 원문 위치를 붙인다.
   - `## 시사이슈 3건` 아래의 각 이슈에는 날짜, 출처, 원문 링크, `#### 핵심 요약`, `#### 중요한 이유`를 넣는다. 발표·보도·추론·미확인 사항을 구분한다.
   - 마지막에 `## 조사·검증 기록`을 두고 검색 질의, 학술 소스·원문 확인 범위, 저장한 PDF, 시사 검색 범위를 기록한다.
   - 각 논문·이슈에 출처, 원문 링크, 핵심 요약, 중요한 이유를 포함하고, 논문마다 허용 카테고리·학회·연도·발표/게재 확인 근거를 명시한다. 한국어 본문과 원어 고유명사 표기를 유지한다.
